classdef (ConstructOnLoad)  Simulation < utils.customdisplay
    % Simulation - Simulation results of EUROMOD tax-benefit system.
    %
    % Syntax:
    %
    %     S = Simulation();
    %
    % Description:
    %     This class contains simulation results of a EUROMOD tax-benefit
    %     system, generated by the method run().
    %
    %  Simulation Properties:
    %     outputs          - (1,N) cell.   A cell array with the table-type simulation results.
    %     settings         - (1,1) struct. A structure with the user-defined settings (i.e. addons, constantsToOverwrite, extensions,..).
    %     output_filenames - (1,N) string. File-names of the simulation output.
    %     errors           - (N,1) string. Errors and warnings from the simulation run.
    %
    %  Example:
