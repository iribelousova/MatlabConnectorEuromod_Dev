classdef TestClass
    ...
    methods
        ...
        function obj=testCountry(obj)
            ...
            obj=getIDs_from_GetTypeInfo(obj,"datasets");
            obj=getIDs_from_GetTypeInfo(obj,"systems");
            obj=getIDs_from_GetTypeInfo(obj,"local_extensions");

            obj=getIDs_from_GetPieceOfInfo(obj,"policies");
        end
        ...
        function obj=testSystem(obj)
            ...
            if all(size(obj.temp)==[1,1])
                ...
                obj=obj.getIDs_from_GetPieceOfInfo("datasets");
                obj=obj.getIDs_from_GetPieceOfInfo("policies");
        ...
        function obj=testDatasetInSystem(obj)
            if all(size(obj.temp)==[1,1])
                ...
                % [new,values]=new.getIDs_from_GetPieceOfInfo("datasets");
    ...
    methods (Access=private)
        ...
        %------------------------------------------------------------------
        function [obj, outValues]=getIDs_from_GetPieceOfInfo(obj,prop)
        ...
        %------------------------------------------------------------------
        function [obj,keys]=getIDs_from_GetTypeInfo(obj,prop)
            msg=[class(obj.temp),' property "',char(prop),'" failed.\n'];
        ...
        function obj=checkIDsList_1(obj,prop,IDtag)
            ...
            if strcmp(prop,"policies")
                [new,values]=new.getIDs_from_GetPieceOfInfo(prop);
            elseif any(strcmp(prop,["datasets","systems"])) && ~contains(class(obj.temp),'InSystem')
                [new,values]=new.getIDs_from_GetTypeInfo(prop);
            else
                [new,values]=new.getIDs_from_GetPieceOfInfo(prop);
            end
